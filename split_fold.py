...
scan_data = pd.read_csv("/media/tungthanhlee/SSD/grand_challenge/dataset/LNDB_segmentation/trainset_csv/trainNodules_gt.csv")
min_vol = min(scan_data["Volume"])
scan_data = scan_data[scan_data["Volume"]>min_vol]
scan_data.drop(["x", "y", "z", "Volume", "Text", "Nodule","RadID","RadFindingID", "AgrLevel"], axis=1, inplace=True)
...
for fold in range(4):
    ...
    # train.to_csv(
    #     f"/home/ad/LungCancer/dataset/split/cubes_csv/train_fold{fold}.csv", index=False)
    
    val_fold = folds[fold].set_index(["LNDbID"])
    val = scan_data.join(val_fold, on=['LNDbID'], how='inner')
    val.to_csv(
        f"/media/tungthanhlee/SSD/grand_challenge/dataset/LNDB_segmentation/trainset_csv/Fold/test_fold/test_fold{fold}.csv", index=False)